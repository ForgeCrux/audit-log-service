...
docker run --rm -p 8080:8080 audit-log-service:local
```

## Manual Cloud Run Deploy

```bash
IMAGE=us-central1-docker.pkg.dev/probestack-prod/probestack-prod-apps/audit-log-service:manual

docker build -t ${IMAGE} .
docker push ${IMAGE}

gcloud run deploy audit-log-service \
  --image=${IMAGE} \
  --platform=managed \
  --region=us-central1 \
